        ...
        <div class="py-12 dark-grunge-bg min-h-screen relative overflow-hidden font-sans selection:bg-red-600 selection:text-white">
            ...
            <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 pt-4 pb-24">
                ...
                <div class="flex justify-between items-center mb-8">
                    <a href="{{ route('albums.index') }}" class="hardcore-link inline-flex items-center text-zinc-500 hover:text-white font-black text-sm uppercase tracking-widest transition-all bg-black/80 px-6 py-3 border border-zinc-800 slanted-button hover:border-red-600 group">
                    ...
                    <a href="{{ route('albums.edit', $album->id) }}" class="hardcore-link inline-flex items-center text-yellow-500 hover:text-black font-black text-sm uppercase tracking-widest transition-all bg-black/80 px-6 py-3 border border-yellow-500 slanted-button hover:bg-yellow-500">
                        OVERRIDE DATA
                    </a>
                    @endif
                </div>
                    ...
                    <div class="flex flex-col md:flex-row gap-10 mt-6 relative z-20">
                        ...
                        <div class="w-full md:w-1/3 flex-shrink-0">
                            <div class="relative w-full aspect-square bg-black p-2 border border-zinc-800 slanted-edge group">
                                ...
                                <img src="{{ $album->cover_image }}" alt="Cover" class="w-full h-full object-cover filter contrast-125 grayscale group-hover:grayscale-0 transition-all duration-700">
                ...
                <div class="mt-12 bg-zinc-950/90 backdrop-blur-xl border-2 border-zinc-800 p-6 md:p-10 slanted-edge shadow-[0_30px_60px_rgba(0,0,0,0.9)] relative overflow-hidden">
                    ...
                    @auth
                    <form action="{{ route('reviews.store', $album->id) }}" method="POST" class="mb-10 bg-black p-6 border border-zinc-800 slanted-edge relative group focus-within:border-red-600 transition-colors shadow-inner">
                        @csrf
                    ...
                    <div class="mb-10 bg-black p-4 border border-zinc-800 slanted-edge text-center">
                        <p class="text-zinc-500 font-mono text-[10px] uppercase tracking-widest">
                            >> YOU MUST <a href="{{ route('login') }}" class="text-red-500 hover:text-white underline">LOGIN</a> TO SUBMIT FEEDBACK <<
                        </p>
                    </div>
                    @endauth
                        ...
                        <div class="bg-black border-l-2 border-zinc-800 hover:border-red-600 p-4 transition-colors relative group slanted-edge">
                            ...
                            <div class="flex justify-between items-start mb-2">
                                <div class="flex items-center gap-2">
                                    <span class="text-red-500 font-black text-sm uppercase drop-shadow-[1px_1px_0px_#450a0a]">{{ $review->user->name }}</span>
                                    <span class="text-zinc-600 font-mono text-[8px] uppercase tracking-widest"> // LOG_{{ sprintf('%04d', $review->id) }}</span>
                                </div>
                                
                                <div class="flex items-center gap-4">
                                    <div class="text-yellow-500 text-xs tracking-widest">
                                        @for($i = 1; $i <= 5; $i++)
                                            @if($i <= $review->rating) ★ @else <span class="text-zinc-800">★</span> @endif
                                        @endfor
                                    </div>

                                    <!-- TOMBOL DELETE (HANYA PEMILIK KOMENTAR ATAU ADMIN) -->
                                    @if(Auth::check() && (Auth::id() == $review->user_id || Auth::user()->role == 'admin'))
                                    <form action="{{ route('reviews.destroy', $review->id) }}" method="POST" class="inline" onsubmit="return confirm('WARNING: PURGE THIS LOG DATA?');">
                                        @csrf
                                        @method('DELETE')
